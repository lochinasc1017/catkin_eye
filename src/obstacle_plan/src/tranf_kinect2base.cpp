...
transf (const sensor_msgs::PointCloud2ConstPtr& input)
{
  ...
  float M_tf[4][4] = {{0.8988, 0.3297, -0.289, 0.86}, {0.4373, -0.6282, 0.6438, -0.541}, {0.0307, -0.705, -0.7087, 0.755}, {0, 0, 0, 1}};  // base_link to kincet_depth_link 旋转矩阵
  // float M_tf[4][4] = {{-0.4373, 0.6281, -0.6438, 0.86}, {0.8988, 0.3297, -0.289, -0.541}, {0.0307, -0.705, -0.7087, 0.755}, {0, 0, 0, 1}};  // base_link to kincet_depth_link 旋转矩阵
  ...
  for (ti=0; ti<po_num; ti++)
  {
    ...
    // Area filter
    if ( 0.3<pn[0] && pn[0]<0.7 && -0.5<pn[1] && pn[1]<0.5 && pn[2]>0 )
    // if ( -0.7<pn[0] && pn[0]<-0.3 && -0.6<pn[1] && pn[1]<0.6 && pn[2]>0 )
    {
      indexs.push_back(ti);
    }
  ...
  scene_pcd.header.frame_id = "base";
...
main (int argc, char** argv)
{
  ...
  ros::Subscriber sub = nh.subscribe ("/pcd_frome_d435i", 18, transf);


  // Create a ROS publisher for the output point cloud
  pub_boject = nh.advertise<sensor_msgs::PointCloud2> ("/scene/points", 5);
