...
class MoveItIkDemo:
    def __init__(self):
        ...
        # 运动位置设置
        sta_xyz = [-0.426, -0.454, 0.35]
        # sta_xyz = [-0.454, 0.426, 0.45]
        sta_rpy = [1.567, 1.469, -3.080]
        goal_xyz = [-0.426, 0.454, 0.35]
        # goal_xyz = [0.454, 0.426, 0.45]
        ...
        # trajectory planning until sucess or reaching number_max
        num_plan_max = 1
        while len(traj.joint_trajectory.points) == 0 and num_plan_max < 25:
            arm.set_start_state_to_current_state()
            arm.set_pose_target(goal_pose, end_effector_link)
            traj = arm.plan()

            print("The %s times attemping!", num_plan_max)
            num_plan_max += 1      
